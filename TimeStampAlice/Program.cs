...
class Program
{
    static void Main(string[] args)
    {
        ...
        do
        {
            ...
            string text = Encoding.GetEncoding(1251).GetString(Encoding.GetEncoding(1251).GetBytes(Console.ReadLine()!));
            string vector = "abcdefgh";

            Console.WriteLine($"Ключ: {key}\nВектор инициализации: {vector}\n");
            DateTime currentTime = DateTime.Now; //Временная метка
            text += "|" + currentTime.ToBinary();
